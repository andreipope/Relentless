...
{   
    ...
    public class Contract : ContractBase<ChainEventArgs> {
        ...
        {
            var tx = CreateContractMethodCallTx(method, args);
            return await CallAsync<T>(tx);
        }

        public async Task<T> CallAsync<T>(string method, IMessage args, Action<BroadcastTxResult> result) where T : IMessage, new()
        {
            var tx = CreateContractMethodCallTx(method, args);
            return await CallAsync<T>(tx, result);
        }
        ...
        private async Task<T> CallAsync<T>(Transaction tx, Action<BroadcastTxResult> callResult) where T : IMessage, new()
        {
            var result = await this.client.CommitTxAsync(tx);
