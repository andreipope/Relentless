    ...
    public class OverlordAbilitiesTests : BaseIntegrationTest
    {
        ...
        public IEnumerator Levitate()
        {
            return AsyncTest(async () =>
            {
                ...
                Action validateEndState = () =>
                {
                    CardModel playerCardInHand = TestHelper.BattlegroundController.GetCardModelByInstanceId(playerWhizperId);
                    CardModel opponentCardInHand = TestHelper.BattlegroundController.GetCardModelByInstanceId(opponentWhizper2Id);
