...
class CZBLoginTests(CZBTests):
    def test_login_invalid_test_key(self):
        ...
        self.altdriver.wait_for_element('InputField_Beta').set_component_property('UnityEngine.UI.InputField','text','123456')
        self.altdriver.find_element('Button_Beta').tap()
        self.altdriver.wait_for_element('WarningPopup(Clone)')
        self.assertTrue(self.altdriver.find_element('Text_Message').get_component_property('TMPro.TextMeshProUGUI','text','Unity.TextMeshPro')=='Input a valid Tester Key')
    ...
    def test_login_valid_test_key(self):
        ...
        self.driver.save_screenshot('./screenshots/term-and-conditions-toggle-checked.png')
