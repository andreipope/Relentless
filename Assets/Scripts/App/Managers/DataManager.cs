...
namespace LoomNetwork.CZB {
    public class DataManager : IService, IDataManager {
        ...
        public void DeleteData() {
            ...
            {
                if (file.Name.Contains("json") || file.Name.Contains("dat") || file.Name.Contains(Constants.VersionFileResolution)) file.Delete();
            }
        ...
        private void CheckVersion() {
            ...
            foreach (FileInfo file in files)
            {
                if (file.Name == BuildMetaInfo.Instance.ShortVersionName + Constants.VersionFileResolution) versionMatch = true;
            }
            ...
            switch (type)
            {
                case Enumerators.CacheDataType.CARDS_LIBRARY_DATA:
                    ...
                    } catch (Exception ex)
                    {
                        ...
                        if (File.Exists(_cacheDataPathes[type])) CachedCardsLibraryData = DeserializeObjectFromPath<CardsLibraryData>(_cacheDataPathes[type]);
                    }
                ...
                case Enumerators.CacheDataType.HEROES_DATA:
                    ...
                    } catch (Exception ex)
                    {
                        ...
                        if (File.Exists(_cacheDataPathes[type])) CachedHeroesData = DeserializeObjectFromPath<HeroesData>(_cacheDataPathes[type]);
                    }

                    break;
                case Enumerators.CacheDataType.USER_LOCAL_DATA:
                    if (File.Exists(_cacheDataPathes[type])) CachedUserLocalData = DeserializeObjectFromPath<UserLocalData>(_cacheDataPathes[type]);
                ...
                case Enumerators.CacheDataType.COLLECTION_DATA:
                    ...
                    } catch (Exception ex)
                    {
                        ...
                        if (File.Exists(_cacheDataPathes[type])) CachedCollectionData = DeserializeObjectFromPath<CollectionData>(_cacheDataPathes[type]);
                    }
                ...
                case Enumerators.CacheDataType.DECKS_DATA:
                    ...
                    try
                    {
                        ...
                        if (listDecksResponse != null)
                        {
                            ...
                            remoteDecksData = new DecksData {
                                Decks = listDecksResponse.Decks.Select(d => JsonConvert.DeserializeObject<Deck>(d.ToString())).ToList()
                            };
                    ...
                    if (localDecksData != null && remoteDecksData != null)
                    {
                        ...
                        } else
                        {
                            ...
                            {
                                CachedDecksData = localDecksData;
                                Debug.LogError("Catastrophe! Error while synchronizing decks, assuming local deck as a fallback");
                    ...
                    break;
                case Enumerators.CacheDataType.DECKS_OPPONENT_DATA:
                    if (File.Exists(_cacheDataPathes[type])) CachedOpponentDecksData = DeserializeObjectFromPath<OpponentDecksData>(_cacheDataPathes[type]);

                    break;
                case Enumerators.CacheDataType.CREDITS_DATA:
                    if (File.Exists(_cacheDataPathes[type])) CachedCreditsData = DeserializeObjectFromPath<CreditsData>(_cacheDataPathes[type]);

                    break;
                case Enumerators.CacheDataType.BUFFS_TOOLTIP_DATA:
                    if (File.Exists(_cacheDataPathes[type])) CachedBuffsTooltipData = DeserializeObjectFromPath<TooltipContentData>(_cacheDataPathes[type]);
        ...
        private T DeserializeObjectFromPath<T>(string path) {
            if (Constants.DataEncryptionEnabled) return JsonConvert.DeserializeObject<T>(Utilites.Decrypt(File.ReadAllText(path), Constants.PrivateEncryptionKeyForApp));

            return JsonConvert.DeserializeObject<T>(File.ReadAllText(path));
        }

        private string SerializeObject(object obj) {
            if (Constants.DataEncryptionEnabled) return Utilites.Encrypt(JsonConvert.SerializeObject(obj, Formatting.Indented), Constants.PrivateEncryptionKeyForApp);
