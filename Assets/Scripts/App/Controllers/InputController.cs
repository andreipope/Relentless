    ...
    public class InputController : IController
    {
        ...
        private void HandleInput()
        {
                ...
                foreach (Touch touch in Input.touches)
                {
                    switch (touch.phase)
                    {
                        case TouchPhase.Began:
                            ...
                            CastRay(touch.position, LayerMask.NameToLayer("Battleground"));
                            _pointerEventSolver.PushPointer(PointerEventSolver.DefaultDelta);
                            break;
                        case TouchPhase.Moved:
                        case TouchPhase.Stationary:
                            CastRay(touch.position, LayerMask.NameToLayer("Battleground"), true);
                            break;
            ...
            else
            {
                ...
                {
                    CastRay(Input.mousePosition, LayerMask.NameToLayer("Battleground"), isHovering: true);
                }
                ...
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    CastRay(Input.mousePosition, LayerMask.NameToLayer("Battleground"));
                    _pointerEventSolver.PushPointer(PointerEventSolver.DefaultDelta);
                }
                else if (Input.GetMouseButton(0))
                {
                    CastRay(Input.mousePosition, LayerMask.NameToLayer("Battleground"), true);
                }
