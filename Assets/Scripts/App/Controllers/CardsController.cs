...
*/
            ...
            Player player, WorkingCard card, BoardObject target, Action<WorkingCard, BoardObject> completePlayCardCallback)
        {
            ...
            SortingGroup group = randomCard.GetComponent<SortingGroup>();
            group.sortingLayerID = SRSortingLayers.Foreground;
            group.sortingOrder = _battlegroundController.OpponentHandCards.FindIndex(x => x == randomCard);
            List<GameObject> allUnitObj = randomCard.GetComponentsInChildren<Transform>().Select(x => x.gameObject).ToList();
